...
export default function ActivityDetailScreen() {
  ...
  async function load() {
    const [{ data: { user } }, { data: act, error }] = await Promise.all([
    ...
    if (error || !act) { setLoading(false); return; }
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            const rsvpResult = await supabase.from('rsvps').delete().eq('activity_id', activity.id);
            console.log('rsvp delete:', JSON.stringify(rsvpResult));
            const actResult = await supabase.from('activities').delete().eq('id', activity.id);
  ...
  const heroImage = activity.image_url ?? CATEGORY_IMAGES[activity.category] ?? 'https://picsum.photos/seed/activity/900/500';
  const hostName = host?.full_name ?? 'Rollin\' User';
  const initials = hostName.trim().split(' ').map(w => w[0]).slice(0, 2).join('').toUpperCase();
    ...
    <AppView style={{ flex: 1 }}>
      ...
      >
        ...
        <View style={styles.heroContainer}>
          <Image source={{ uri: heroImage }} style={styles.hero} contentFit="cover" cachePolicy="memory-disk" />
        ...
        <View style={styles.body}>
          ...
          >
            {hostAvatarUrl ? (
              ...
              <View style={[styles.hostAvatar, styles.hostInitials, { backgroundColor: colors.tint }]}>
                <AppText style={[styles.initialsText, { color: colors.onImageOverlay, fontFamily: Fonts?.sans }]}>
                  {initials}
                </AppText>
              </View>
            )}
          ...
          {activity.description ? (
            <View style={styles.descSection}>
              <AppText style={[styles.sectionHeading, { color: colors.text, fontFamily: Fonts?.sans }]}>About</AppText>
...
const styles = StyleSheet.create({
  ...
  metaDivider: { height: StyleSheet.hairlineWidth, backgroundColor: 'rgba(0,0,0,0.08)' },
  descSection: { gap: 8 },
