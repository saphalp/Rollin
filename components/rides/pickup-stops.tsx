...
export function PickupStops({ rideId, route, refresh }: { rideId: string; route: RoadRoute | null; refresh: (optimize?: boolean) => Promise<void> }) {
  ...
  return <View style={{ gap: 14 }}>
    <AppText style={{ fontSize: 18, fontWeight: '700' }}>Remaining stops</AppText>
    <AppText>While parked, return here to mark each passenger picked up. Then open Google Maps for your next stop.</AppText>
    {loadError ? <TouchableOpacity onPress={() => void action(load)}><AppText>{loadError}</AppText></TouchableOpacity> : null}
    {stops.map((s, i) => <View key={s.requestId ?? 'destination'} style={{ gap: 8, paddingVertical: 8 }}>
    ...
    {stops.length > 0 && <TouchableOpacity disabled={busy} onPress={() => void action(() => openGoogleMaps(stops, false))}>
      <AppText style={{ color: '#165FD5', fontWeight: '700' }}>Navigate to next stop</AppText>
    </TouchableOpacity>}
    {pickups.length > 1 && <TouchableOpacity disabled={busy} onPress={() => void action(() => refresh(true))}>
      <AppText style={{ color: '#165FD5' }}>{busy ? 'Updating…' : 'Recalculate pickup order'}</AppText>
    </TouchableOpacity>}
  </View>;
}
